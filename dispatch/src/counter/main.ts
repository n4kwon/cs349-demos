...
// very simple app state (i.e. the "model")
let counter = 0;
...
increaseButton.addEventListener("action", (_) => {
  console.log("incrementButton action!");
  counter++;
  textfield.text = counter.toString();
});

clearButton.addEventListener("action", (_) => {
...
textfield.addEventListener("textchanged", (e) => {
  // the event source property provides a reference
  // to the widget that triggered the event
